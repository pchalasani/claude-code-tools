...
| Plugin | Description |
|--------|-------------|
| [`visual-brief`][visual-brief] | Optional milestone reminders for Visual Brief |
...
| `agent-tunnel` | `>share` a live session so teammates can ask it questions over Discord or Mattermost |
...
`visual-brief`, `dynamic-workflow`, `voxtype`, and `writing` plugins. Add it
once, then install with `codex plugin add`:
...
Visual Brief requires its separate CLI, installed with `uv tool install
visual-brief`. Its optional plugin adds quiet, session-scoped reminders after a
successful briefing publish. See the [Visual Brief reference][visual-brief] for
the local browser behavior and setup.
...
[visual-brief]: /claude-code-tools/plugins-detail/visual-brief/

---

## Visual Brief Plugin

Visual Brief turns substantial Claude Code or Codex updates into concise local
browser briefings. Install the `visual-brief` CLI separately, then optionally
install this plugin for quiet milestone reminders. See the
[Visual Brief reference][visual-brief] for the complete setup and workflow.
...
questions over Discord or Mattermost — each answered against a private,
read-only **fork**.
...
setup: creating the Discord or Mattermost bot, configuration, and running the
daemon.
...
Three prose-quality skills, including Jev feedback during writing. The original
two skills remain explicit-trigger only.
...
| `/jev-prose` | Jev pattern checks and up to three guided revisions |

The original two skills vendor a pinned snapshot of their upstream source
...
MIT), so they work offline. Jev checks need the separate `jev-prose` CLI and
a configured backend; see the detail page below.

See the
[Writing detail page](/claude-code-tools/plugins-detail/writing/)
for installation in both agents, the skills' scope, the detector
