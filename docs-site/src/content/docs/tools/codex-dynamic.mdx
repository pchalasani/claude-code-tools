...
marker to the TUI and configures it in the managed App Server's
shell-environment policy, where tool processes are actually created. The
default therefore survives restrictive settings such as `inherit = "core"`. The
...
## Monitor workflows with codex-workflows
...
## Plugin updates and server generations

Codex refreshes installed plugin caches inside the long-running app server and
invalidates its own in-memory plugin state when the effective plugins change.
Those cache files and Codex-maintained marketplace fields such as
`last_updated` and `last_revision` are therefore runtime state, not immutable
app-server inputs.
...
The launcher fingerprints the selected Codex executable, user-selected plugin
and marketplace configuration, plugin feature gates, and server-affecting
options. If they match an existing healthy generation, it reuses that
generation. Otherwise it starts and certifies a new socket before making it the
default. Existing TUIs and in-flight callbacks remain connected to the old
socket, so they are not interrupted. Codex-managed plugin caches, catalog
downloads, sync timestamps, and observed marketplace revisions do not create
new generations.

When resuming an explicit session ID or name, the launcher first asks every
compatible managed generation which threads it already has loaded. If an older
generation owns that thread, the new TUI reconnects to that socket instead of
trying to load a second in-memory copy on the current generation. A bare resume
picker and the `--last` form stay on the current generation because the launcher
cannot safely reproduce Codex's native selection rules. After a rollover, use
the explicit session ID or name if that thread is still loaded by an older
generation. Older generations are eligible only when their Codex executable,
version, server options, and process-level plugin configuration match the
current configuration.

The launcher reads configuration through a race-checked descriptor. If a user
or Codex rewrites `config.toml` during that read, the launcher retries with a
small fixed limit. Cache refreshes can continue independently without blocking
or rolling the shared server.
...
1. **Establish a callback-capable session**
   ...
   fingerprints that executable together with process-level plugin
   configuration and server options. It reuses the matching healthy generation
   or starts a supervised app server on a
   generation-specific Unix socket. The current generation marker changes only
   after the new listener passes ownership, process-identity, plugin-snapshot,
   and socket-peer checks. A failed rollout therefore leaves the previous
   default untouched.
   ...
   configured in the managed App Server's shell-environment policy, making
   callbacks the default for detached workflow launches even under a
   restrictive inheritance policy.
