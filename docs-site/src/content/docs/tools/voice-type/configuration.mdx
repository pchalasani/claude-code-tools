...
3. **Whole-take** — `segmentation = "hold"`.
   ...
   take is transcribed as one segment and typed as
   one atomic paste. Maximum accuracy — the model
   sees full sentence context — and one Cmd-Z undoes
   it. Requires `toggle` mode, since the wake word
