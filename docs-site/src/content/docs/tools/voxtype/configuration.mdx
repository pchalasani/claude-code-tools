...
|--------|---------|--------|-------|
| `parakeet-mlx` | Apple GPU (MLX) | ~40x realtime | **Recommended.** fp16 Parakeet-TDT v3: best accuracy and speed. Apple Silicon only; weights from HuggingFace on first run. Extra: `mlx` |
| `parakeet` | CPU (sherpa-onnx) | ~32x realtime | int8 Parakeet-TDT v3 (or fp16 v2 via `parakeet_model`); ~490 MB download. Extra: `parakeet` |
| `moonshine` | CPU (ONNX) | streaming | Moonshine streaming models; smallest footprint. Installed by default |
