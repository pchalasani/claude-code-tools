...
CGEvent tap, and draws its overlay with AppKit. On
**Apple Silicon** (M1 or later) it defaults to the
`parakeet-mlx` Apple-GPU engine; Intel Macs
automatically fall back to the `parakeet` CPU
engine (`moonshine` ships no Intel-macOS build).
:::
