...
handles, threads, access levels, and attachments. Codex has no in-session
`>share` hook, so you publish from the terminal instead:

```bash
cd ~/work/payments        # the project the codex session ran in
agent-tunnel share --agent codex payments-gpt
# Sharing session 019f6c23… [codex] as: payments-gpt
```

By default it publishes the **newest codex session recorded in that
directory** (pass `--session <id>` to pick another, and the same
`--write` / `--dangerously-allow-bash` / `--dangerously-skip-permissions`
flags to set access). `agent-tunnel published` and Discord's `!list` tag
codex handles with `[codex]`.

Two codex-specific mechanics worth knowing:
